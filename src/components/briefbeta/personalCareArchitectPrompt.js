...
- Slide 2: global trend overview with slide_type "table", preheader "GLOBAL TRENDS | BEAUTY & PERSONAL CARE", columns ["Trend", "Global signal", "Evidence"], and one row per selected trend. Ground each row in that trend's verified market_signal and sources; do not mention regional GNPD as global proof. Add one supporting_data entry per row using that trend's exact [SRC:…] source_id so the overview remains traceable.
- Slide 3: regional trend overview with slide_type "table", preheader "REGIONAL TRENDS | <REGION>", columns ["Trend", "Regional expression", "Launch proof"], and one row per selected trend. Ground every regional expression in that trend's in-region GNPD products, and begin each Launch proof cell with the exact GNPD Record ID.
