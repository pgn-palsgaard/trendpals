...
- Inventing or reconstructing a citation: any supporting_data source not present word-for-word in the evidence below is a fabrication and rejects the deck, even if the publisher is real and the claim is true.
...
- A supporting_data "source" MUST be copied VERBATIM from a source title, inline citation, or web signal shown in the evidence below. Never paraphrase it, never reconstruct a title from a URL slug, never combine a real publisher with an invented title, and never name a publisher you happen to know (GreyB, ConfectioneryNews, Mintel, any research house) unless that exact item appears in the evidence. If no shown source supports a point, omit the supporting_data entry entirely — an unverifiable citation is worse than none. Every citation is checked verbatim against the retrieved evidence at save time; one untraceable citation rejects the WHOLE deck and forces a rewrite.
