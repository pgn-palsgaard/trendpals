...
To stay inside the body budget, cap every content slide at: market_signal 2 sentences, hypothesis_tieback 1 line (max 130 characters), why_it_may_matter 2 sentences, 2-3 formulation questions, max 2 supporting_data entries, max 4 gnpd_examples (each one line, max ~140 characters), 1 conversation opener. Count the characters before you emit — the budget covers ALL of these combined. Section divider subtitles must be ~6 words. Agenda items are one line each, max 130 characters.
...
<slides>
[{"slide_number": 1, "slide_name": "short internal name", "slide_type": "content", "category": "one canonical category key", "trend_id": "the exact TREND ID of the single verified trend this slide is built on, copied from the evidence block", "evidence_class": "regional or read_across — regional unless this slide is built on the trend's CROSS-REGION REFERENCE products", "title": "...", "subtitle": "...", "market_signal": "1-2 sentences, external market signals only", "hypothesis_tieback": "ONE line, max 130 characters, stating how THIS trend advances the core hypothesis — observation framing, mandatory on every trend slide", "why_it_may_matter": "the commercial implication for a manufacturer deciding now — observation, never instruction", "formulation_questions": ["the technical question the observation raises"], "supporting_data": [{"stat": "...", "source_id": "the [SRC:…] or [WEB:…] tag copied verbatim from the evidence — never a citation string"}], "gnpd_examples": ["<GNPD Record ID> | Product name — Brand (Country): one-line why it evidences the trend"], "conversation_openers": ["one open question"]}]
</slides>
Deck structure:
- Slide 1 = opening slide framing the core hypothesis (the red thread across ALL categories).
- Slide 2 = agenda slide: {"slide_type": "agenda", "title": "one insight-framed overview title", "subtitle": "one line", "agenda_items": ["TREND 01 — <trend name>: half-line on its angle", ...]} — EXACTLY one item per trend slide, in deck order, numbered to match the implications preheaders (TREND 01, TREND 02 …), each item max 130 characters. No products, no citations, no other fields.
...
- DRIVER GROUPING (hard rule): within each category, order the fully evidenced trend slides by their PRIMARY DRIVER exactly as the drivers appear in the evidence (the trends arrive pre-sorted by driver — keep that order). All trends sharing a driver stay contiguous — never interleave drivers. The SIGNAL and CROSS-REGION tiers still come after the FULL tier; driver grouping applies within each tier.
- HYPOTHESIS TIE-BACK (hard rule): every trend slide carries "hypothesis_tieback" — one line, max 130 characters, that explicitly connects this trend's observation to the core hypothesis, so the red thread runs through the middle of the deck, not only its ends. Observation framing; never repeat the hypothesis verbatim.
